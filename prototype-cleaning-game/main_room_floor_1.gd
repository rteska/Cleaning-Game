...
var Main_room_floor_1_cleaned = false
var alreadyGenerated = false

var total = 35
var total_score = 35
var main_room_floor_1_score = 0
...
func _on_main_spawn_bacteria() -> void:
	...
	if !Main_room_floor_1_cleaned && !alreadyGenerated:
...
func _on_leave_main_room_floor_1_hide_bacteria() -> void:
	for child in get_children():
		for grandchildren in child.get_children():
			if grandchildren is Area2D:
	...
	if total == 0 && !score_added:
		...
		#Globals.score += main_room_floor_1_score
		score_added = true
	pass_points.emit(main_room_floor_1_score, total_score, 0, 0, self)

func add_score():
	main_room_floor_1_score += 1
	Globals.score += 1
	total -= 1
