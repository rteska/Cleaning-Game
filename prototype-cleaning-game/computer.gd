...
@export var computer_bl = Node2D
@export var computer_tr = Node2D

var centerPos = global_position
var Computer_cleaned = false
...
func _on_main_spawn_bacteria() -> void:
	...
	if !Computer_cleaned && !alreadyGenerated:
		for i in range(5): #Create a bacteria cluster on the screen
			var bacteria = bacteriaPath.instantiate()
			
			bacteria.rotate_random()
			bacteria.position = Vector2(randf_range(computer_bl.position.x, computer_tr.position.x), randf_range(computer_bl.position.y, computer_tr.position.y))
...
func _on_leave_computer_hide_bacteria() -> void:
	for child in get_children():
		for grandchildren in child.get_children():
			if grandchildren is Area2D:
	...
	if total == 0 && !score_added:
		...
		#Globals.score += Computer_score
		score_added = true
	pass_points.emit(Computer_score, total_score, 0, 0, self)
